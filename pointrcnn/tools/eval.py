...
def clean_data(gt_anno, dt_anno, current_class, difficulty):
    ...
    eval_by_dist = bool(difficulty / 3)
    difficulty %= 3
    dist_boundary = np.array([[0, 30, 50],
                              [30, 50, 70]])
    ...
    for i in range(num_gt):
        ...
        if (gt_name == current_cls_name):
            ...
            valid_class = -1
        ignore = False
        if (not eval_by_dist) and ((gt_anno["occluded"][i] > MAX_OCCLUSION[difficulty])
                or (gt_anno["truncated"][i] > MAX_TRUNCATION[difficulty])
                or (height <= MIN_HEIGHT[difficulty])):
            # if gt_anno["difficulty"][i] > difficulty or gt_anno["difficulty"][i] == -1:
            ignore = True
        if eval_by_dist and ((gt_anno["occluded"][i] > MAX_OCCLUSION[-1])
                or (gt_anno["truncated"][i] > MAX_TRUNCATION[-1])
                or (height <= MIN_HEIGHT[-1])):
            ignore = True

        if eval_by_dist and not (dist_boundary[0, difficulty] < gt_anno["location"][i, 2] < dist_boundary[1, difficulty]):
            ignored_gt.append(-1)
        elif valid_class == 1 and not ignore:
            ignored_gt.append(0)
            num_valid_gt += 1
    ...
    for i in range(num_dt):
        ...
        if eval_by_dist and not (dist_boundary[0, difficulty] < dt_anno["location"][i, 2] < dist_boundary[1, difficulty]):
            ignored_dt.append(-1)
        elif (eval_by_dist and height < MIN_HEIGHT[difficulty]) or (not eval_by_dist and height < MIN_HEIGHT[-1]):
            ignored_dt.append(1)
        elif valid_class == 1:
            ignored_dt.append(0)
        else:
